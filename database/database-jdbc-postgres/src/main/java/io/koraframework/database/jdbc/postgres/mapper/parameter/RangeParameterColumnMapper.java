...
public class RangeParameterColumnMapper<T> implements JdbcParameterColumnMapper<Range<T>> {
    ...
    // Непустую границу квотим: timestamp содержит пробел, а любой bound со спецсимволом (запятая, скобки)
    // иначе сломает парсинг range-литерала. Пустая граница (±infinity) остаётся без кавычек.
